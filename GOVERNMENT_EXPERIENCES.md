...
Reviewer lifecycle commands are also local and staging-only. Suspension must be performed by a different active administrator, can never reduce the registry below two active administrators, and revokes every live Identity session and unused invitation for the target. Reactivation likewise requires a different active administrator and deliberately requires a new invitation before access resumes. Pending onboarding requests are marked expired before the queue is displayed, and the content-free administration history can be inspected without exposing invitation or session secrets.
